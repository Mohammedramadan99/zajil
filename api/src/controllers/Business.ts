...
export const BusinessController: ICRUDController = {
    create: function (req: RequestMod, res: Response, next: NextFunction): void {
        ...
        businessServices
            ...
            .then((buessiness) => res.status(201).json(buessiness))
            .catch((err) => {
                console.error(err);
    ...
    update: function (req: Request, res: Response, next: NextFunction): void {
        const buessinessId = Number(req.params.id);
        const body: UpdateBusinessDto = validateDto(UpdateBusinessDto, req.body);

        businessServices
            .updateBusinessById(buessinessId, body)
            .then((buessiness) => res.json(buessiness))
            .catch((err) => {
                console.error(err);
                if (err instanceof HttpError) next(err);
                else next(new HttpError(500, err.message));
    ...
    delete: function (req: Request, res: Response, next: NextFunction): void {
        ...
        businessServices
            .deleteBusinessById(buessinessId)
            .then((buessiness) => res.json(buessiness))
            .catch((err) => {
                console.error(err);
                if (err instanceof HttpError) next(err);
